class speedometer_dashboard {
	...
	constructor(container, data, labels_dashboard, opts) {
    	...
    	small_speed_width = Math.floor(chart_width / 3);

    	let unique_technologies = d3.map(data.filter(d => d.ald_sector == sector), d => d.technology).keys();

    	function createSmallDialDivs(div_id) {
    		let dial_div = d3.select('.chart_div')
    		.append('div')
    			.attr('id', div_id)
    	...
    	let port_dial_div = d3.select('.chart_div')
    			.append('div')
    			.attr('id', 'portfolio_dial')
    			.attr('class', 'main_dial')
    			.style('width', width + 'px')
    			
    	for (let i = 1; i <= 8; i++) {
		  createSmallDialDivs('tech' + i + '_dial', unique_technologies[i - 1]);
		}

		function insertGauge(div_id, data, width, height, sector, technology, portfolio_name, benchmark_name, chart_title) {
			...
			function updateReadings(data, isMainPortfolio, portfolioName, sector, technology) {
				...
				if (isMainPortfolio) {
					powerGauge.update_portfolio(subdata[0].tdm_technology_value);
				} else {
					powerGauge.update_benchmark(subdata[0].tdm_technology_value);
				}
			}
		...
		insertGauge('portfolio_dial', data, main_speed_width, main_speed_height, 'Aggregate', 'Aggregate', portfolio_name, null, 'Portfolio result');
		
		for (let i = 1; i <= 8; i++) {
		  insertGauge('tech' + i + '_dial', data, small_speed_width, small_speed_height, sector, unique_technologies[i - 1], portfolio_name, null, unique_technologies[i - 1]);
		}
